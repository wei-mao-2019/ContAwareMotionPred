...
from pvcnn.modules import SharedMLP, PVConv, PointNetSAModule, PointNetAModule, PointNetFPModule

__all__ = ['create_mlp_components', 'create_pointnet_components',
           'create_pointnet2_sa_components', 'create_pointnet2_fp_modules']


def _linear_bn_relu(in_channels, out_channels, is_bn=True):
    if is_bn:
        return nn.Sequential(nn.Linear(in_channels, out_channels), nn.BatchNorm1d(out_channels), nn.ReLU(True))
    else:
        return nn.Sequential(nn.Linear(in_channels, out_channels), nn.ReLU(True))
...
def create_mlp_components(in_channels, out_channels, classifier=False, dim=2, width_multiplier=1,
    ...
    if dim == 1:
        block = functools.partial(_linear_bn_relu,is_bn=is_bn)
    else:
        block = functools.partial(SharedMLP, is_bn=is_bn)
    ...
    if dim == 1:
        if classifier:
            ...
            layers.append(_linear_bn_relu(in_channels, int(r * out_channels[-1]),is_bn=is_bn))
    else:
        if classifier:
            layers.append(nn.Conv1d(in_channels, out_channels[-1], 1))
        else:
            layers.append(SharedMLP(in_channels, int(r * out_channels[-1]), is_bn=is_bn))
...
def create_pointnet2_sa_components(sa_blocks, extra_feature_channels, with_se=False, normalize=True, eps=0,
                                   width_multiplier=1, voxel_resolution_multiplier=1, is_bn=True):
    r, vr = width_multiplier, voxel_resolution_multiplier
    ...
    for conv_configs, sa_configs in sa_blocks:
        ...
        sa_blocks = []
        if conv_configs is not None and len(conv_configs) > 0:
            ...
            if voxel_resolution is None:
                ...
                block = functools.partial(PVConv, kernel_size=3, resolution=int(vr * voxel_resolution),
                                          with_se=with_se, normalize=normalize, eps=eps,is_bn=is_bn)
            for _ in range(num_blocks):
        ...
        if num_centers is None:
            block = functools.partial(PointNetAModule,is_bn=is_bn)
        else:
            block = functools.partial(PointNetSAModule, num_centers=num_centers, radius=radius,
                                      num_neighbors=num_neighbors, is_bn=is_bn)
...
def create_pointnet2_fp_modules(fp_blocks, in_channels, sa_in_channels, with_se=False, normalize=True, eps=0,
    ...
    for fp_idx, (fp_configs, conv_configs) in enumerate(fp_blocks):
        ...
        fp_blocks.append(
            PointNetFPModule(in_channels=in_channels + sa_in_channels[-1 - fp_idx],
                             out_channels=out_channels, is_bn=is_bn)
        )
